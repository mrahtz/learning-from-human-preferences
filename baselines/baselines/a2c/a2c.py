...
class Runner(object):
    ...
    def __init__(self, env, model, seg_pipe, log_dir, nsteps=5, nstack=4, gamma=0.99,
            orig_rewards=False, gen_segs=True):
        self.env = env
        self.model = model
    ...
    def gen_segments(self, mb_obs, mb_dones):
        ...
        for step in range(self.nsteps):
            if len(self.segment) == 25:
                #print("New segment generated")
                self.seg_pipe.put(self.segment)
                self.segment = []
                continue
            elif e0_dones[step]:
                # The last segment will probably not be 25 steps long;
                # drop it, so that all segments in the batch are the same length
                self.segment = []
                continue
...
def learn(policy, env, seed, seg_pipe, go_pipe, log_dir, nsteps=5, nstack=4,
        total_timesteps=int(80e6), vf_coef=0.5, ent_coef=0.01,
        max_grad_norm=0.5, lr=7e-4, lrschedule='linear', epsilon=1e-5,
        alpha=0.99, gamma=0.99, log_interval=100, load=False,
        save_interval=10000, orig_rewards=False, gen_segs=True):
    ...
    make_model = lambda: Model(policy=policy, ob_space=ob_space, ac_space=ac_space, nenvs=nenvs, nsteps=nsteps, nstack=nstack, num_procs=num_procs, ent_coef=ent_coef, vf_coef=vf_coef, max_grad_norm=max_grad_norm, lr=lr, alpha=alpha, epsilon=epsilon, total_timesteps=total_timesteps, lrschedule=lrschedule)
    ...
    model = make_model()
    runner = Runner(env, model, seg_pipe, log_dir, nsteps=nsteps, nstack=nstack,
            gamma=gamma, orig_rewards=orig_rewards, gen_segs=gen_segs)
